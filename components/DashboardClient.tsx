...
export function DashboardClient({ repos, userLogin, autoConfig }: Props) {
  // 惰性初始化：直接从 autoConfig 计算初始值，避免在 useEffect 里调 setState
  const [selectedRepos, setSelectedRepos] = useState<string[]>(() => {
    if (!autoConfig?.autoRepo) return [];
    const repo = repos.find(
      (r) => r.name === autoConfig.autoRepo || r.full_name.endsWith("/" + autoConfig.autoRepo)
    );
    return repo ? [repo.full_name] : [];
  });
  const [provider, setProvider] = useState<AIProvider>(() => {
    if (autoConfig?.autoProvider === undefined) return null;
    return autoConfig.autoProvider === "null" ? null : (autoConfig.autoProvider as AIProvider);
  });
  const [selectedModel, setSelectedModel] = useState<string | null>(() => {
    if (!autoConfig?.autoProvider || autoConfig.autoProvider === "null") return null;
    const p = autoConfig.autoProvider as NonNullable<AIProvider>;
    return autoConfig.autoModel || defaultModel(p);
  });
  ...
  // 自动触发生成（仅执行一次，初始 selectedRepos 已由惰性初始化设好）
  useEffect(() => {
    if (
      autoConfig?.autogenerate !== "1" ||
